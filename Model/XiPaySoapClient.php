...
//include_once ("XiPaySoapOpTemplate.php");
//include_once ("TransactionResponse.php");
//
//XiPaySoapClient - call XiPayWS from this class 
//
class XiPaySoapClient extends \Tng\Paymetric\Model\NTLMSoapClient {
    ...
    function __construct($XiPayServiceURL, $XiPayUser, $XiPayUserPassword){
        ...
        stream_wrapper_register('http', '\Tng\Paymetric\Model\XiPaySoapClient') or die("Failed to register protocol");
    ...
    private function SoapOp($transaction)
    {
        ...
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/paymetric_soapop.log');
		$logger = new \Zend\Log\Logger();
		$logger->addWriter($writer); //Use $logger->info() to log items to cc_debug.log
        $logger->info('');
        
        try{
        	$logger->info("TRY START. PREPARING TRANSACTION");
            ...
            $logger->info("tryna log trans response");
            $logger->info(print_r($transactionResponse));
        }catch(\Exception $e){
        	
            $transactionResponse = new \Tng\Paymetric\Model\TransactionResponse();
    ...
    private function DoRequest($request){
       ...
       // $this->trace("", $response);
        ...
        if (!isset($xml->Body->Fault)) {
            $this->trace("SOAP OP RESULT", $xml->Body->SoapOpResponse->SoapOpResult->packets->ITransactionHeader->StatusCode . ","); 
            return $xml->Body->SoapOpResponse->SoapOpResult;
        }else{
          //soap error
          $this->trace("SOAP ERROR", $xml); 
